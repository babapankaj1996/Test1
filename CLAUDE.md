...
- Stack: Next.js 16 App Router + TypeScript + Tailwind 4 + Node SQLite (no ORM). DB auto-creates and seeds at `data/blog.db` on first `getDb()` call (`lib/db.ts`).
- Public pages live in `app/(site)/`, admin in `app/admin/` (login page is outside the guarded `(panel)` route group). All pages are `force-dynamic`; data access is synchronous SQLite in server components.
- Every `/api/admin/*` handler must call `guardAdmin()` from `lib/api-helpers.ts` first. Write handlers must pass the request object so same-origin checks run. Auth = jose JWT in an httpOnly cookie; `AUTH_SECRET` is required in production and `data/.auth-secret` is local-dev only.
...
- Image uploads go through `/api/admin/upload`: sharp-validated, re-encoded to WebP into `data/uploads/`.
